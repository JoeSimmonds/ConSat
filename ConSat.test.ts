import {
    backtrack,
    Constraint,
    ConstraintSet, identityIdRoundTrip,
    NO_CONSTRAINT,
    PredicateConstraint
} from "../src/ConSat";
...
test("binds a result to a value from the domain", () => {
    const domain = [1,6,7,12]

    const variables: [string, Iterable<number>][] = [["blah", domain]]


    const results: [string, number][][] = backtrack<string, number>(variables, NO_CONSTRAINT, identityIdRoundTrip)
...
test('selects a value that doesn\'t contradict the constraints', () => {
    const domain:number[] = [1,6,7,12]

    const variables: [string, Iterable<number>][] = [["blah", domain]]

    const results = backtrack<string, number>(
...
test('All fields are assigned a value', ()=> {
    const variables: [string, Iterable<number|string>][] = [
        ['foo', [1,5,7,8,12,134]],
        ['bar', ['monday', 'tuesday', 'Wednesday']]
    ]

    const results = backtrack<string, number|string>(variables, NO_CONSTRAINT, identityIdRoundTrip)
...
test('Multiple fields can be checked in a constraint', ()=> {
    const variables: [string, Iterable<number|string>][] = [
        ['foo', [1,5,7,8,12,134]],
        ['bar', ['monday', 'tuesday', 'Wednesday']]
    ]

    const results = backtrack<string, number|string>(variables, new PredicateConstraint<string, number|string>(
...
test('empty domain returns an empty response indicating no valid solutions', () =>{
    const domain:number[] = []
    ...
    const results = backtrack<string, number>(
        variables, NO_CONSTRAINT, identityIdRoundTrip
    )

    expect(results.length).toBe(0)
})

test('returns no results if no satisfying combination can be found', ()=> {
    const variables: [string, Iterable<number|string>][] = [
        ['foo',[1,5,7,12,134]],
    ...
    const results = backtrack<string, number|string>(variables, new PredicateConstraint<string, number|string>(
        "foo must contain the length of bar",
        (c: [string, (number | string)][]) => new Map(c).get('foo') === (new Map(c).get('bar') as string).length,
        c => new Map(c).has('foo') && new Map(c).has('bar')
     ), identityIdRoundTrip)
